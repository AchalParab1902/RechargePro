...
import { FaTimes, FaThLarge, FaMobileAlt, FaTags, FaUser, FaSignOutAlt } from 'react-icons/fa';
...
const UserSidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    { title: 'Dashboard', path: '/user-dashboard', icon: <FaThLarge /> },
    { title: 'Instant Recharge', path: '/user-dashboard/recharge', icon: <FaMobileAlt /> },
    { title: 'Browse Plans', path: '/user-dashboard/plans', icon: <FaTags /> },
    { title: 'Profile', path: '/user-dashboard/profile', icon: <FaUser /> },
  ];
      ...
      <aside className={`w-64 bg-slate-900 min-h-screen text-slate-300 flex flex-col fixed left-0 top-0 z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-20 flex items-center justify-between px-6 border-b border-slate-800">
          <div className="flex items-center">
            ...
            <span className="text-white font-black text-xs tracking-[0.2em] uppercase italic opacity-90">User Portal</span>
          </div>
